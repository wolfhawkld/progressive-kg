...
PCA 可以从[[协方差矩阵]]特征分解或中心化数据的 SVD 两条等价路径理解；两者都先处理数据相对均值的变化。总体与样本协方差的定义、分母和半正定性质见独立页。
...
| [[奇异值分解|SVD]] | 线性 | 无监督 | 矩阵分解 UΣVᵀ | PCA 的计算工具 |
...
- 相关：[[奇异值]] — 中心化数据的奇异值平方对应主成分方差的尺度

- 相关：[[表征学习]] — PCA 从方差结构学习线性表示

- 前置：[[特征值与特征向量]] — 协方差矩阵的特征向量与特征值给出方向和方差
- [[奇异值分解|SVD]] — PCA 的数值计算工具，更稳定
- [[标准化|标准化(Standardization)]] — 特征量纲不宜直接比较时先缩放；PCA 本身只要求中心化
- [[内积]] — PCA 投影 = 数据向量与主成分的内积
- [[正交]] — 主成分之间严格正交
- [[均方误差]] — PCA 用均方误差最小化寻找最优投影
- 相关：[[协方差矩阵]] — 通过协方差矩阵的特征方向降维
- 相关：[[正交投影]] — 主成分子空间上的低维投影
- 相关：[[自编码器]] — 线性平方重构在特定条件下与 PCA 有联系
...
- 2026-09-11：SVD入口改指独立分解页，保留奇异值关系；保留原 verified。
