...
归一化后点积就是 [[余弦相似度]]。它使相似度主要表示方向关系；没有归一化或使用不同投影头时，数值不能直接与这里的 logits 定义互换。

## 对称的图文对比目标

一个 batch 含 $N$ 个正确配对，矩阵中的对角线是正样本，其余位置是当前 batch 提供的负样本。CLIP 同时训练“图像找文本”和“文本找图像”两个方向。

这个按 batch 识别配对样本、带温度缩放的目标与 [[InfoNCE]] 风格形式相近；CLIP 原论文将它写成图像到文本和文本到图像的对称交叉熵。其训练数据包含自然语言描述，因此不能仅凭没有类别标签就把 CLIP 归为完全无标签的 [[自监督学习]]。
...
- 相关：[[InfoNCE]] — 双向批内匹配使用相近的温度对比目标，但数据配对和目标表述仍遵循 CLIP 协议
- 相关：[[自监督学习]] — 图文配对提供训练信号，但原始 CLIP 使用带自然语言语义的网页配对
- 相似度：[[余弦相似度]] — 归一化图像和文本向量的点积等于余弦相似度
