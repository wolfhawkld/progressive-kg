...
3. **检索素材（多路来源，强制）**：不要只靠内部知识；也不要在"用户没给源"或"觉得这是教科书常识"时跳过检索。四路并行，缺哪一路都可能出问题：
   - **web_search（必做，不受用户是否提供源影响）**——三个目的，任一都足以让它成为必需：
     - **发现**：确认概念是否真实存在、有没有更通用的名称或缩写。防止"A 是 B 的别名""内部知识里想当然"这类错误
     - **时效**：这个概念是不是很新？近期是否改名、版本迭代、被推翻？**内部知识有截止时间，对近期发布的模型/框架/论文，不搜就可能写成过期版本，甚至不知道它存在**
     - **覆盖**：有没有比通用百科更合适的一手源（官方文档、原始论文、权威综述）
   - **用户指定的参考源（如有）**：优先级高于泛搜结果；用户给的源不好用时说明理由并回退到其他路
   - **本地库**：搜索 raw/ 目录、human_ai_knowledge（如果存在）、已有概念节点（查重 + 可复用关系）
   - **web_extract 取正文**：对选定 URL 抓正文。**只验证 URL 可达不构成核验**
...
- **只验证 URL 可达 ≠ 核验** -- `curl -w %{http_code}` 拿到 200 只说明链接存在。给概念页标 `verified` 之前，必须能指出论断来自哪一段**被抓取的正文**；只验链接的页面不得标 verified
- **搜索结果不可全信** -- SEO 农场、AI 生成内容、过时页面都会混入（例如 Wikipedia 条目本身也曾被打上 suspected AI-generated texts 标签）。多路交叉比对，冲突时以一手源（原始论文、官方文档）为准，并在页面里记录分歧
- **很新的概念必须靠 search** -- 内部知识有截止时间。对近期发布的模型/框架/论文先搜再写：搜索至少要能确认"它是否存在、标准名称是什么、最新版本是哪个"
- **"教科书常识"也要过一遍 search** -- 判据不是"我觉得这很基础"，而是"这个论断我能否指出具体正文出处"。不能指出就搜
